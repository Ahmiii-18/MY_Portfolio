...
$(document).on('DOMContentLoaded', function () {
    ...
    if (ityped && window.ityped && window.ityped.init) {
        window.ityped.init(ityped, {
            strings: ['WEB DEVELOPER AND UI DESIGNER', 'WEB DESIGNER', 'FREELANCER', 'PHOTOGRAPHER'],
            loop: true
        });
    }
});
...
/*----------------------- Slides Js -----------------------------*/
const slider = document.getElementById("horizontalWrapper");
let isDown = false;
let startX;
let scrollLeft;
...
if (slider) {
    slider.addEventListener("mousedown", (e) => {
        isDown = true;
        slider.classList.add("dragging", "no-select");
    ...
    document.addEventListener("mouseup", () => {
        isDown = false;
        slider.classList.remove("dragging", "no-select");
...
function syncClientImages(swiperInstance) {
    if (!swiperInstance || !swiperInstance.slides || !swiperInstance.activeIndex) return;
    ...
    document.querySelectorAll('.client-images-gp img').forEach(img => {
        img.classList.toggle('active', img.getAttribute('src') === activeSrc);
    });
}
